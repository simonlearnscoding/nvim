--- ### simon Status API
--
-- Statusline related utility functions to use within simon and user configurations.
--
-- This module can be loaded with `local status = require "simon.utils.status"`
--
-- @module simon.utils.status
-- @copyright 2022
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("simon.utils.status").hl.mode },
...
-- @usage local heirline_component = require("heirline.utils").surround({ "|", "|" }, require("simon.utils.status").hl.mode_bg, heirline_component),
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.fileicon(), hl = require("astronvim.utils.status").hl.filetype_color },
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("simon.utils.status").hl.get_attributes("treesitter") },
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("simon.utils.status").hl.file_icon("winbar") },
...
-- @usage local heirline_component = { init = require("simon.utils.status").init.breadcrumbs { padding = { left = 1 } } }
function M.init.breadcrumbs(opts)
  ...
  return function(self)
    ...
    if opts.max_depth and opts.max_depth > 0 then
      ...
      if start_idx > 0 then
        table.insert(children, { provider = require("simon.utils").get_icon "Ellipsis" .. opts.separator })
      end
    end
...
---@return function # The Heirline init function
-- @usage local heirline_component = { init = require("simon.utils.status").init.separated_path { padding = { left = 1 } } }
...
---@return function # The Heirline init function
-- @usage local heirline_component = { init = require("simon.utils.status").init.update_events { "BufEnter", { "User", pattern = "LspProgressUpdate" } } }
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.fill }
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.signcolumn }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.numbercolumn }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.foldcolumn }
-- @see simon.utils.status.utils.stylize
function M.provider.foldcolumn(opts)
  opts = extend_tbl({ escape = false }, opts)
  local ffi = require "simon.utils.ffi" -- get AstroNvim C extensions
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.tabnr() }
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.spell() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.paste() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.macro_recording() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.showcmd() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.search_count() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.mode_text() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.percentage() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.scrollbar() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.close_button() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.filetype() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.filename() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.unique_path() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.file_modified() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.file_read_only() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.file_icon() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.git_branch() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.git_diff({ type = "added" }) }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.diagnostics({ severity = "ERROR" }) }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.lsp_progress() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.treesitter_status() }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("simon.utils.status").provider.str({ str = "Hello" }) }
-- @see simon.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.is_active }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = function() return require("simon.utils.status").condition.buffer_matches { buftype = { "terminal" } } end }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.is_macro_recording }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.is_hlsearch }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.is_statusline_showcmd }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.is_git_repo }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.git_changed }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.file_modified }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.file_read_only }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.has_diagnostics }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.has_filetype }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.aerial_available }
...
---@param bufnr table|integer a buffer number to check the condition for, a table with bufnr property, or nil to get the current buffer
---@return boolean # whether or not LSP is attached
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.lsp_attached }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("simon.utils.status").condition.treesitter_available }
...
-- @usage local string = require("simon.utils.status").utils.stylize("Hello", { padding = { left = 1, right = 1 }, icon = { kind = "String" } })
...
-- @usage local heirline_component = require("simon.utils.status").component.fill()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.file_info()
...
---@param opts? table options for configuring file_icon, filename, filetype, file_modified, file_read_only, and the overall padding
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.tabline_file_info()
function M.component.tabline_file_info(opts)
  return M.component.file_info(extend_tbl({
    ...
    close_button = {
      ...
      on_click = {
        callback = function(_, minwid) require("simon.utils.buffer").close(minwid) end,
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.nav()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.cmd_info()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.mode { mode_text = true }
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.breadcumbs()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.separated_path()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.git_branch()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.git_diff()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.diagnostics()
...
---@param opts? table options for configuring diagnostic providers and the overall padding
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.treesitter()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.lsp()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.foldcolumn()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.numbercolumn()
...
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").component.signcolumn()
...
--- A general function to build a section of simon status providers with highlights, conditions, and section surrounding
---@param opts? table a list of components to build into a section
---@return table # The Heirline component table
-- @usage local heirline_component = require("simon.utils.status").components.builder({ { provider = "file_icon", opts = { padding = { right = 1 } } }, { provider = "filename" } })
...
-- @usage local heirline_component = { on_click = { callback = function(...) local args = require("simon.utils.status").utils.statuscolumn_clickargs(...) end } }
...
M.heirline.make_buflist = function(component)
  ...
  return require("heirline.utils").make_buflist(
    ...
    function() return vim.t.bufs end, -- use simon bufs variable
