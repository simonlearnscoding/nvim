--- ### simon Buffer Utilities
--
-- Buffer management related utility functions
--
-- This module can be loaded with `local buffer_utils = require "simon.utils.buffer"`
--
-- @module simon.utils.buffer
-- @copyright 2022
...
function M.move(n)
  ...
  require("simon.utils").event "BufsUpdated"
...
function M.close(bufnr, force)
  ...
  if require("simon.utils").is_available "bufdelete.nvim" then
...
---@param compare_func string|function a string of a comparator defined in require("simon.utils.buffer").comparator or a custom comparator function
...
function M.sort(compare_func, skip_autocmd)
  ...
  if type(compare_func) == "function" then
    ...
    vim.t.bufs = bufs
    if not skip_autocmd then require("simon.utils").event "BufsUpdated" end
...
local function unique_path(bufnr)
  return require("simon.utils.status").provider.unique_path() { bufnr = bufnr }
